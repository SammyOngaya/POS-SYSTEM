...
namespace POS_SYSTEM
{
    public partial class DebtorPayDebt : Form
    {
        ...
        public DebtorPayDebt(string pfno, string databaseString)
        {
            InitializeComponent();
        ...
        public void searchStockID()
        {
            ...
            try
            {
                ...
                MySqlCommand com = new MySqlCommand("SELECT `product`.`name` AS 'name' FROM `product` JOIN `stock` ON `product`.`id`=`stock`.`product_id` JOIN `debtor` ON `stock`.`stock_id`=`debtor`.`stock_id` WHERE (((`debtor`.`quantity`*`debtor`.`price`)>`debtor`.`deposit`) AND (`debtor`.`quantity` > 0)) GROUP BY `debtor`.`stock_id` ORDER BY `product`.`name` ASC", con);
        ...
        public void searchPhone()
        {
            ...
            try
            {
                ...
                MySqlCommand com = new MySqlCommand("SELECT phone FROM  debtor WHERE (((`debtor`.`quantity`*`debtor`.`price`)>`debtor`.`deposit`) AND (`debtor`.`quantity` > 0))  ORDER BY phone ASC", con);
        ...
        public void viewDebtorFilterByPhone()
        {
            try
            {
                ...
                MySqlCommand com = new MySqlCommand("SELECT `debtor`.`id` AS 'DEBTOR ID',`debtor`.`name` AS 'NAME',`product`.`name` AS 'PRODUCT NAME',`debtor`.`quantity` AS 'QUANTITY',`debtor`.`price` AS 'PRICE',(`debtor`.`quantity`*`debtor`.`price`) AS 'AMOUNT',((`debtor`.`quantity`*`debtor`.`price`)-`debtor`.`deposit`) AS 'BALANCE',`debtor`.`date_borrowed` AS 'DATE BORROWED',`debtor`.`phone` AS 'PHONE',`debtor`.`deposit` AS 'DEPOSIT',`debtor`.`date_of_payment` AS 'DATE OF PAYMENT',`debtor`.`pfno` AS 'REGISTERED BY',`debtor`.`registered_date` AS 'REGISTRATION DATE' ,`debtor`.`stock_id` AS 'STOCK ID' FROM `product` JOIN `stock` ON `product`.`id`=`stock`.`product_id` JOIN `debtor` ON `stock`.`stock_id`=`debtor`.`stock_id` WHERE ( ((`debtor`.`quantity`*`debtor`.`price`)>`debtor`.`deposit`) AND (`debtor`.`phone`='" + this.phoneTextBox.Text + "') AND (`debtor`.`quantity` > 0))  ORDER BY `debtor`.`phone` ASC", con);
        ...
        public void debtorPdf()
        {
            ...
            try
            {
                ...
                PdfWriter PW = PdfWriter.GetInstance(doc, new FileStream("C:\\POS\\Reports\\DEBTOR REQUEST REPORT", FileMode.Create));
                ...
                System.Diagnostics.Process.Start("C:\\POS\\Reports\\DEBTOR REQUEST REPORT");
        ...
        public void debtorsExcel()
        {
            ...
            try
            {
                ...
                ExcelLibrary.DataSetHelper.CreateWorkbook("C:\\POS\\Reports\\DEBTOR REQUEST REPORT.xls", ds);

                MessageBox.Show("Report generated successfully");
                Process sts = System.Diagnostics.Process.Start("C:\\POS\\Reports\\DEBTOR REQUEST REPORT.xls");
                sts.WaitForExit();
        ...
        public void showDebtorData()
        {
            try
            {
                ...
                MySqlCommand com = new MySqlCommand("SELECT `debtor`.`id` AS 'debtor_id',`debtor`.`stock_id` AS 'stock_id',`debtor`.`name` AS 'debtorname',`product`.`name` AS 'productname',`debtor`.`quantity` AS 'drquantity',`debtor`.`price` AS 'price',`debtor`.`phone` AS 'phone' FROM `product` JOIN `stock` ON `product`.`id`=`stock`.`product_id` JOIN `debtor` ON `stock`.`stock_id`=`debtor`.`stock_id` WHERE (( `debtor`.`id`='" + this.searchDebtorIDTextBox.Text + "') AND (`debtor`.`quantity` > 0)) ", con);
                ...
                while (r.Read())
                {
                    ...
                    string productid = r.GetString("stock_id");
                    ...
                    stockIDTextBox.Text = productid;
                    ProductNameLabel.Text = "PRODUCT NAME : " + dr + " ";
        ...
        public void ReverseStockQuantity()
        {
            try
            {
                ...
                mc.ExecuteNonQuery();
                MessageBox.Show("PRODUCT SUCCESSFULLY REVERSED", "PRODUCT REVERSAL", MessageBoxButtons.OK, MessageBoxIcon.Information);
                con.Close();
            }
            catch (Exception)
            {
        ...
        public void SetDebtorToZeroAfterReversal()
        {
            try
            {
                ...
                mc.ExecuteNonQuery();
                //MessageBox.Show("PRODUCT SUCCESSFULLY REVERSED", "DRUPRODUCTG REVERSAL", MessageBoxButtons.OK, MessageBoxIcon.Information);
                con.Close();
            }
            catch (Exception)
            {
        ...
        private void button6_Click_1(object sender, EventArgs e)
        {
            if (searchDebtorIDTextBox.Text != string.Empty && updateDebtorPrimaryKey.Text != string.Empty)
            {
                ...
                dialog = MessageBox.Show("Are you sure you want to make the following changes?", "product REVERSAL", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
        ...
        private void userAmountTextBox_TextChanged(object sender, EventArgs e)
        {
            if (System.Text.RegularExpressions.Regex.IsMatch(userAmountTextBox.Text, "[^ 0-9]"))
            {
                userAmountTextBox.Text = "";
