...
namespace POS_SYSTEM
{
    public partial class ReportDailySales : Form
    {
        ...
        public ReportDailySales(string databaseString)
        {
            InitializeComponent();
        ...
        public void discountView()
        {
            ...
            {
                con.Open();
                MySqlCommand discount = new MySqlCommand("SELECT DISTINCT(serial), SUM(discount) FROM product JOIN stock ON `product`.`id`=`stock`.`product_id` JOIN supplier ON `supplier`.`id`=`stock`.`supplier_id` JOIN sales ON `sales`.`stock_id`=`stock`.`stock_id` GROUP BY sales_date ", con);
        ...
        public void discountViewFilterByDateRange()
        {
            ...
            try
            {
                ...
                con.Open();
                MySqlCommand discount = new MySqlCommand("SELECT DISTINCT(serial), SUM(discount) FROM product JOIN stock ON `product`.`id`=`stock`.`product_id` JOIN supplier ON `supplier`.`id`=`stock`.`supplier_id` JOIN sales ON `sales`.`stock_id`=`stock`.`stock_id` WHERE sales_date BETWEEN '" + startDate + "' AND '" + endDate + "' GROUP BY sales_date ", con);
        ...
        public void discountViewFilterByDate()
        {
            ...
            try
            {
                ...
                con.Open();
                MySqlCommand discount = new MySqlCommand("SELECT DISTINCT(serial),  SUM(discount) FROM product JOIN stock ON `product`.`id`=`stock`.`product_id` JOIN supplier ON `supplier`.`id`=`stock`.`supplier_id` JOIN sales ON `sales`.`stock_id`=`stock`.`stock_id` WHERE sales_date='" + date + "' GROUP BY sales_date ORDER BY sales_date DESC", con);
        ...
        public void salesExcel()
        {
            ...
            try
            {
                ...
                ExcelLibrary.DataSetHelper.CreateWorkbook("C:\\POS\\Reports\\DAILY SALES REPORT.xls", ds);

                MessageBox.Show("Report generated successfully...............!", "DAILY SALES REPORT", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Process sts = System.Diagnostics.Process.Start("C:\\POS\\Reports\\DAILY SALES REPORT.xls");
                sts.WaitForExit();
        ...
        public void salesPdf()
        {
            try
            {
                ...
                PdfWriter PW = PdfWriter.GetInstance(doc, new FileStream("C:\\POS\\Reports\\DAILY SALES REPORT.pdf", FileMode.Create));
                ...
                System.Diagnostics.Process.Start("C:\\POS\\Reports\\DAILY SALES REPORT.pdf");
