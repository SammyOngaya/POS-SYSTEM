...
namespace POS_SYSTEM
{
    public partial class UpdateOrderList : Form
    {
        ...
        DialogResult dialog;
        public UpdateOrderList(string user, string databaseString)
        {
            InitializeComponent();
        ...
        public void searchDrug()
        {
            ...
            try
            {
                ...
                MySqlCommand sqlCommand = new MySqlCommand("SELECT `product`.`name` AS 'name' FROM `product` JOIN `supplier_price_list` ON `product`.`id`=`supplier_price_list`.`product_id` WHERE `product`.`status`='Active' GROUP BY `supplier_price_list`.`product_id` ORDER BY `product`.`name` ASC", con);
        ...
        public void supplierData()
        {
            ...
            try
            {
                ...
                MySqlCommand sqlCommand = new MySqlCommand("SELECT `supplier`.`name` AS 'name',`order_price_list`.`supplier_id` AS 'supplier_id' FROM `supplier` JOIN `order_price_list` ON `supplier`.`id`=`order_price_list`.`supplier_id`  WHERE `supplier`.`name`='" + this.searchSupplierTextBox.Text + "' GROUP BY `order_price_list`.`product_id`", con);
                MySqlDataReader reader = sqlCommand.ExecuteReader();
                while (reader.Read())
                {
                    String name = reader.GetString("name");
        ...
        public void quantityData()
        {
            ...
            try
            {
                ...
                MySqlCommand sqlCommand = new MySqlCommand("SELECT `quantity` FROM `order_price_list`  WHERE (`order_price_list`.`supplier_id`='" + this.supplieridforeignkey.Text + "' AND `order_price_list`.`product_id`='" + this.drugidforeignkey.Text + "')", con);
                MySqlDataReader reader = sqlCommand.ExecuteReader();
                while (reader.Read())
                {
        ...
        private void orderListGridView()
        {
            try
            {
                ...
                MySqlCommand com = new MySqlCommand("SELECT `supplier`.`name` AS 'SUPPLIER',`product`.`name` AS 'PRODUCT',`order_price_list`.`price` AS 'PRICE', `order_price_list`.`quantity` AS 'QUANTITY',ROUND((`order_price_list`.`price`*`order_price_list`.`quantity`),2) AS 'TOTAL' FROM `supplier` JOIN `order_price_list` ON `supplier`.`id`=`order_price_list`.`supplier_id` JOIN `product` ON `product`.`id`=`order_price_list`.`product_id`  ORDER BY `supplier`.`name` ASC", con);
        ...
        private void minimumPriceList()
        {
            try
            {
                ...
                MySqlConnection con = new MySqlConnection(db);
                con.Open();
                MySqlCommand com = new MySqlCommand("SELECT `supplier`.`name` AS 'SUPPLIER',`product`.`name` AS 'PRODUCT',`min_order_price_list`.`price` AS 'PRICE', `min_order_price_list`.`quantity` AS 'QUANTITY',ROUND((`min_order_price_list`.`price`*`min_order_price_list`.`quantity`),2) AS 'TOTAL' FROM `supplier` JOIN `min_order_price_list` ON `supplier`.`id`=`min_order_price_list`.`supplier_id` JOIN `product` ON `product`.`id`=`min_order_price_list`.`product_id`  ORDER BY `supplier`.`name` ASC", con);
                MySqlDataAdapter a = new MySqlDataAdapter();
                a.SelectCommand = com;
        ...
        public void minimalOrderPriceList()
        {
            ...
            System.Diagnostics.Process.Start("C:\\poss\\Reports\\Minimal Order List pdf");
        ...
        {
            string db = databaseConnectionStringTextBox.Text;
            if (searchProductTextBox.Text == string.Empty || searchSupplierTextBox.Text == string.Empty || supplieridforeignkey.Text == string.Empty || drugidforeignkey.Text == string.Empty || quantityTextBox.Text == string.Empty)
            {
            ...
            else
            {
                ...
                try
                {
                    ...
                    dialog = MessageBox.Show("Are you sure you want to remove the product?", "ORDER LIST DELETE", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                    ...
                    if (dialog == DialogResult.Yes)
                    {
                        ...
                        MySqlCommand c = new MySqlCommand("DELETE FROM `order_price_list` WHERE (`order_price_list`.`supplier_id`='" + this.supplieridforeignkey.Text + "' AND `order_price_list`.`product_id`='" + this.drugidforeignkey.Text + "')", con);

                        c.ExecuteNonQuery();

                        // delete the minimal order list first.
                        MySqlCommand minimalOrderList = new MySqlCommand("DELETE FROM `min_order_price_list`  WHERE (`min_order_price_list`.`supplier_id`='" + this.supplieridforeignkey.Text + "' AND `min_order_price_list`.`product_id`='" + this.drugidforeignkey.Text + "')", con);
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (dialog == DialogResult.Yes)
                    {
                        ...
                        MySqlCommand c = new MySqlCommand("UPDATE order_price_list SET `order_price_list`.`quantity` ='" + this.quantityTextBox.Text + "'  WHERE (`order_price_list`.`supplier_id`='" + this.supplieridforeignkey.Text + "' AND `order_price_list`.`product_id`='" + this.drugidforeignkey.Text + "')", con);

                        c.ExecuteNonQuery();

                        // Update the minimal order list first.
                        MySqlCommand minimalOrderList = new MySqlCommand("UPDATE min_order_price_list SET `min_order_price_list`.`quantity` ='" + this.quantityTextBox.Text + "'  WHERE (`min_order_price_list`.`supplier_id`='" + this.supplieridforeignkey.Text + "' AND `min_order_price_list`.`product_id`='" + this.drugidforeignkey.Text + "')", con);
