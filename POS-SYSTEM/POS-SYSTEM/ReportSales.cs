...
namespace POS_SYSTEM
{
    public partial class ReportSales : Form
    {
        ...
        DataTable dataTable;
        DialogResult dialog;
        public ReportSales(string databaseString)
        {
            InitializeComponent();
        ...
        public void discountView()
        {
            ...
            try
            {
                ...
                con.Open();
                MySqlCommand discount = new MySqlCommand("SELECT DISTINCT(`sales`.`serial`) , discount FROM product JOIN stock ON `product`.`id`=`stock`.`product_id` JOIN supplier ON `supplier`.`id`=`stock`.`supplier_id` JOIN sales ON `sales`.`stock_id`=`stock`.`stock_id` ", con);
         ...
         public void discountViewFilterByDateRange()
        {
            ...
            try
            {
                MySqlConnection con = new MySqlConnection(db);
                con.Open();
                MySqlCommand discount = new MySqlCommand("SELECT DISTINCT(`sales`.`serial`) , discount FROM product JOIN stock ON `product`.`id`=`stock`.`product_id` JOIN supplier ON `supplier`.`id`=`stock`.`supplier_id` JOIN sales ON `sales`.`stock_id`=`stock`.`stock_id` WHERE sales_date BETWEEN '" + startDate + "' AND '" + endDate + "' ", con);
         ...
         public void discountViewFilterByDate()
        {
            ...
            try
            {
                MySqlConnection con = new MySqlConnection(db);
                con.Open();
                MySqlCommand discount = new MySqlCommand("SELECT DISTINCT(`sales`.`serial`) , discount FROM product JOIN stock ON `product`.`id`=`stock`.`product_id` JOIN supplier ON `supplier`.`id`=`stock`.`supplier_id` JOIN sales ON `sales`.`stock_id`=`stock`.`stock_id` WHERE sales_date='" + date + "' ORDER BY sales_date DESC", con);
         ...
         public void discountViewFilterByStaff()
        {
            ...
            try
            {
                MySqlConnection con = new MySqlConnection(db);
                con.Open();
                MySqlCommand discount = new MySqlCommand("SELECT DISTINCT(`sales`.`serial`) , discount FROM product JOIN stock ON `product`.`id`=`stock`.`product_id` JOIN supplier ON `supplier`.`id`=`stock`.`supplier_id` JOIN sales ON `sales`.`stock_id`=`stock`.`stock_id` WHERE `sales`.`pfno`='" + this.staffIDTextBox.Text + "' ", con);
         ...
         public void discountViewFilterByDrugName()
        {
            ...
            try
            {
                MySqlConnection con = new MySqlConnection(db);
                con.Open();
                MySqlCommand discount = new MySqlCommand("SELECT DISTINCT(`sales`.`serial`) , discount FROM product JOIN stock ON `product`.`id`=`stock`.`product_id` JOIN supplier ON `supplier`.`id`=`stock`.`supplier_id` JOIN sales ON `sales`.`stock_id`=`stock`.`stock_id` WHERE `product`.`name`='" + this.drugNameTextBox.Text + "' ", con);
         ...
         public void discountViewFilterBySerial()
         {
             ...
             try
             {
                 MySqlConnection con = new MySqlConnection(db);
                 con.Open();
                 MySqlCommand discount = new MySqlCommand("SELECT DISTINCT(`sales`.`serial`) , discount FROM product JOIN stock ON `product`.`id`=`stock`.`product_id` JOIN supplier ON `supplier`.`id`=`stock`.`supplier_id` JOIN sales ON `sales`.`stock_id`=`stock`.`stock_id` WHERE serial='" + this.serialNoTextBox.Text + "' ", con);
         ...
         { 
             ...
             try
             {
                 MySqlConnection con = new MySqlConnection(db);
                 con.Open();
                 MySqlCommand discount = new MySqlCommand("SELECT DISTINCT(`sales`.`serial`) , discount FROM product JOIN stock ON `product`.`id`=`stock`.`product_id` JOIN supplier ON `supplier`.`id`=`stock`.`supplier_id` JOIN sales ON `sales`.`stock_id`=`stock`.`stock_id` WHERE (sales_date='" + date + "' AND `sales`.`pfno`='" + this.staffIDTextBox.Text + "') ", con);
         ...
         public void discountViewFilterByDateAndDrug()
         {
             ...
             try
             {
                 MySqlConnection con = new MySqlConnection(db);
                 con.Open();
                 MySqlCommand discount = new MySqlCommand("SELECT DISTINCT(`sales`.`serial`) , discount FROM product JOIN stock ON `product`.`id`=`stock`.`product_id` JOIN supplier ON `supplier`.`id`=`stock`.`supplier_id` JOIN sales ON `sales`.`stock_id`=`stock`.`stock_id` WHERE (sales_date='" + date + "' AND `product`.`name`='" + this.drugNameTextBox.Text + "') ", con);
         ...
         public void discountViewFilterByDateRangeAndDrug()
         {
             ...
             try
             {
                 MySqlConnection con = new MySqlConnection(db);
                 con.Open();
                 MySqlCommand discount = new MySqlCommand("SELECT DISTINCT(`sales`.`serial`) , discount FROM product JOIN stock ON `product`.`id`=`stock`.`product_id` JOIN supplier ON `supplier`.`id`=`stock`.`supplier_id` JOIN sales ON `sales`.`stock_id`=`stock`.`stock_id` WHERE ((sales_date BETWEEN '" + startDate + "' AND '" + endDate + "') AND (`product`.`name`='" + this.drugNameTextBox.Text + "')) ", con);
        ...
        public void searchDrug()
        {
            ...
            try
            {
                ...
                MySqlCommand c = new MySqlCommand("SELECT `product`.`name` AS 'name' FROM product JOIN stock ON `product`.`id`=`stock`.`product_id` JOIN supplier ON `supplier`.`id`=`stock`.`supplier_id` JOIN sales ON `sales`.`stock_id`=`stock`.`stock_id` GROUP BY `sales`.`stock_id` ORDER BY `product`.`name` ASC", con);
        ...
        public void salesViewFilterByDateAndDrug()
        {
            ...
            try
            {
                ...
                MySqlCommand com = new MySqlCommand("SELECT sales_id AS 'SALE ID',`sales`.`stock_id` AS 'STOCK ID',`stock`.`batch_no` AS 'BATCH NO.', `product`.`name` AS 'DRUG NAME',`sales`.`quantity` AS 'QUANTITY',`stock`.`buying_price` AS 'BUYING PRICE',`sales`.`price` AS 'SELLING PRICE',ROUND((`sales`.`quantity` *`stock`.`buying_price`),2) AS 'BUYING AMOUNT',ROUND((`sales`.`quantity` *`sales`.`price`),2) AS 'SELLING AMOUNT',((ROUND((`sales`.`quantity` *`sales`.`price`),2))-(ROUND((`sales`.`quantity` *`stock`.`buying_price`),2))) AS 'PROFIT', `sales`.`pfno` AS 'SERVED BY',sales_date AS 'DATE SOLD',`stock`.`units` AS 'UNITS', serial AS 'RECEIPT NUMBER',`supplier`.`name` AS 'SUPPLIER',`stock`.`expiry_date` AS 'EXPIRY DATE' FROM product JOIN stock ON `product`.`id`=`stock`.`product_id` JOIN supplier ON `supplier`.`id`=`stock`.`supplier_id` JOIN sales ON `sales`.`stock_id`=`stock`.`stock_id` WHERE (sales_date='" + date + "' AND `product`.`name`='" + this.drugNameTextBox.Text + "') ORDER BY sales_date DESC", con);
        ...
        public void salesViewFilterByDateRangeAndDrug()
        {
            ...
            try
            {
                ...
                MySqlCommand com = new MySqlCommand("SELECT sales_id AS 'SALE ID',`sales`.`stock_id` AS 'STOCK ID',`stock`.`batch_no` AS 'BATCH NO.', `product`.`name` AS 'DRUG NAME',`sales`.`quantity` AS 'QUANTITY',`stock`.`buying_price` AS 'BUYING PRICE',`sales`.`price` AS 'SELLING PRICE',ROUND((`sales`.`quantity` *`stock`.`buying_price`),2) AS 'BUYING AMOUNT',ROUND((`sales`.`quantity` *`sales`.`price`),2) AS 'SELLING AMOUNT',((ROUND((`sales`.`quantity` *`sales`.`price`),2))-(ROUND((`sales`.`quantity` *`stock`.`buying_price`),2))) AS 'PROFIT', `sales`.`pfno` AS 'SERVED BY',sales_date AS 'DATE SOLD',`stock`.`units` AS 'UNITS', serial AS 'RECEIPT NUMBER',`supplier`.`name` AS 'SUPPLIER',`stock`.`expiry_date` AS 'EXPIRY DATE' FROM product JOIN stock ON `product`.`id`=`stock`.`product_id` JOIN supplier ON `supplier`.`id`=`stock`.`supplier_id` JOIN sales ON `sales`.`stock_id`=`stock`.`stock_id` WHERE ((sales_date BETWEEN '" + startDate + "' AND '" + endDate + "') AND (`product`.`name`='" + this.drugNameTextBox.Text + "')) ORDER BY sales_date DESC", con);
        ...
        public void salesViewFilterByDrugName()
        {
            try
            {
                ...
                MySqlCommand com = new MySqlCommand("SELECT sales_id AS 'SALE ID',`sales`.`stock_id` AS 'STOCK ID',`stock`.`batch_no` AS 'BATCH NO.', `product`.`name` AS 'DRUG NAME',`sales`.`quantity` AS 'QUANTITY',`stock`.`buying_price` AS 'BUYING PRICE',`sales`.`price` AS 'SELLING PRICE',ROUND((`sales`.`quantity` *`stock`.`buying_price`),2) AS 'BUYING AMOUNT',ROUND((`sales`.`quantity` *`sales`.`price`),2) AS 'SELLING AMOUNT',((ROUND((`sales`.`quantity` *`sales`.`price`),2))-(ROUND((`sales`.`quantity` *`stock`.`buying_price`),2))) AS 'PROFIT', `sales`.`pfno` AS 'SERVED BY',sales_date AS 'DATE SOLD',`stock`.`units` AS 'UNITS', serial AS 'RECEIPT NUMBER',`supplier`.`name` AS 'SUPPLIER',`stock`.`expiry_date` AS 'EXPIRY DATE' FROM product JOIN stock ON `product`.`id`=`stock`.`product_id` JOIN supplier ON `supplier`.`id`=`stock`.`supplier_id` JOIN sales ON `sales`.`stock_id`=`stock`.`stock_id` WHERE `product`.`name`='" + this.drugNameTextBox.Text + "' ORDER BY sales_date DESC", con);
        ...
        public void salesExcel()
        {
            ...
            try
            {
                ...
                ExcelLibrary.DataSetHelper.CreateWorkbook("C:\\POS\\Reports\\SALES REPORT.xls", ds);

                MessageBox.Show("Report generated successfully...............!", "SALES REPORT", MessageBoxButtons.OK, MessageBoxIcon.Information);
                Process sts = System.Diagnostics.Process.Start("C:\\POS\\Reports\\SALES REPORT.xls");
                sts.WaitForExit();
        ...
        public void salesPdf()
        {
            ...
            PdfWriter PW = PdfWriter.GetInstance(doc, new FileStream("C:\\POS\\Reports\\SALES REPORT", FileMode.Create));
            ...
            System.Diagnostics.Process.Start("C:\\POS\\Reports\\SALES REPORT");
            ...
            try
            {
                    ...
                    decimal netStockInitialQuantity = Convert.ToDecimal(reverseInitialQuantityTextBox.Text);
                     ...
                     if ((netStockQuantity + qty) <= netStockInitialQuantity)
                     {
                      ...
                      {
                          MessageBox.Show("The product seems not to exist from the system", "NO PRODUCT MATCH FOUND", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                      }
        ...
        public void queryNetStockValue()
        {
            ...
            {
               MessageBox.Show("YOU CAN NOT REVERSE THE SELECTED PRODUCT. PLEASE CHECK IF THE PRODUCT WAS TAKEN BY THE DEBTOR. ELSE YOU CAN CLOSE THE APPLICATION AND TRY AGAIN THE PROCESS","DRUG REVERSAL ERROR",MessageBoxButtons.OK,MessageBoxIcon.Information);
            }
            finally
            {
                //
        ...
        private void button9_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    MessageBox.Show("NO PRODUCT SELECTED. PLEASE SELECT THE DRUG TO PROCEED.", "NO PRODUCT SELECTED", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show("NO PRODUCT SELECTED. PLEASE SELECT THE PRODUCT TO PROCEED.", "NO PRODUCT SELECTED", MessageBoxButtons.OK, MessageBoxIcon.Question);
        ...
        private void staffIDTextBox_TextChanged_1(object sender, EventArgs e)
        {
           // searchStaff();
        ...
        private void searchByStaffID_Click(object sender, EventArgs e)
        {
            searchStaff();
            discountViewFilterByStaff();
            salesViewFilterByStaff();
        ...
        private void serialNoTextBox_TextChanged_1(object sender, EventArgs e)
        {
           // searchSerial();
            discountViewFilterBySerial();
            salesViewFilterBySerial();
